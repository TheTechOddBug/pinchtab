...
func allTools() []mcp.Tool {
	return []mcp.Tool{
		...
		mcp.NewTool("pinchtab_snapshot",
			...
			mcp.WithString("selector", mcp.Description("Unified selector to scope the snapshot (CSS, XPath, text, or ref). Selectors resolve in the current frame scope; use pinchtab_frame for iframe content.")),
		...
		mcp.NewTool("pinchtab_screenshot",
			...
			mcp.WithString("tabId", mcp.Description("Target tab ID")),
			mcp.WithString("selector", mcp.Description("Unified selector to capture a specific element (CSS, XPath, text, or ref). Selectors resolve in the current frame scope.")),
			...
			mcp.WithBoolean("css1x", mcp.Description("When selector is set, return element screenshot at CSS pixel size (1x) instead of device pixels")),
		),
		...
		mcp.NewTool("pinchtab_click",
			...
			mcp.WithString("selector", mcp.Description("Unified selector: ref (e.g. 'e5'), CSS ('css:#btn' or '#btn'), XPath ('xpath://button'), text ('text:Submit'), or semantic ('find:login button'). Non-ref selectors resolve in the current frame scope.")),
